...
def run_test(cmd, is_reference):
    ...
    try:
        ...
        print(e)
        print("%s solution failed correctness check!" %
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run task system performance tests')
    ...
    print("==============================================================="
          "=================")
    print("Running task system grading harness... (%d total tests)" %
          len(test_names_and_num_threads))
    print("  - Detected CPU with %d execution contexts" %
          multiprocessing.cpu_count())
    print("  - Task system configured to use at most %d threads" %
          args.num_threads)
    print("==============================================================="
          "=================")
    ...
    for (test_name, num_threads) in test_names_and_num_threads:
        ...
        if platform.system() == 'Darwin':
            ...
            if platform.machine() == "arm64":
                ...
                ref_cmd = "./%s_osx_arm -n %d" % (
                    REFERENCE_BINARY_NAME, num_threads)
            else:
                print("Reference binary: ./runtasks_ref_osx_x86")
                ref_cmd = "./%s_osx_x86 -n %d" % (
                    REFERENCE_BINARY_NAME, num_threads)
        else:
            if platform.machine() == "aarch64":
                print("Reference binary: ./runtasks_ref_linux_arm")
                ref_cmd = "./%s_linux_arm -n %d" % (
                    REFERENCE_BINARY_NAME, num_threads)
            else:
